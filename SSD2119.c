...
#define RCGCGPIO         (*((volatile uint32_t *) 0x400FE608)) // page 340
#define RCGC2            (*((volatile uint32_t *) 0x400FE108)) // page 464

#define GPIO_PORTB_DIR   (*((volatile uint32_t *) 0x40005400)) // page 663
#define GPIO_PORTB_AFSEL (*((volatile uint32_t *) 0x40005420)) // page 671
#define GPIO_PORTB_DEN   (*((volatile uint32_t *) 0x4000551C)) // page 683

#define GPIO_PORTA_DATA  (*((volatile uint32_t *) 0x40005000)) // page 662
#define GPIO_PORTA_DIR   (*((volatile uint32_t *) 0x40004400)) // page 663
#define GPIO_PORTA_AFSEL (*((volatile uint32_t *) 0x40004420)) // page 671
#define GPIO_PORTA_DEN   (*((volatile uint32_t *) 0x4000451C)) // page 683

void LCD_GPIOInit(void){
    unsigned long wait = 0;
    
     /*SYSCTL_RCGC2_R = 0x20; // enable PORTF for GPIOs
    GPIO_PORTF_DIR_R = 0x0E; // this sets the LEDs at output (those are set to 1) and sets PF0 and PF1 as input (0)
    // even though Blue is not being used, it must enabled as an output, otherwise it's an input and circuit acts weird
    GPIO_PORTF_DATA_R = 0;
    GPIOPUR_PORTF_R = 0x11;      // GPIO_PORTF_DEN_R + RED_GREEN; // set switch and LED bits on the pull-up control register
    GPIOLOCK_PORTF_R = 0x4C4F434B; // unlocking PF0 input pin
    GPIOCR_PORTF_R = 0xFF; // uncomitting all bits
    GPIO_PORTF_DEN_R = 0x1F; // enable port F as digital - all bits*/
    
    
                                           // activate parallel data pins
    RCGCGPIO |= 0x02;                      // activate port B
    RCGC2 |= 0x02;
    wait++;                                // wait for port activation
    wait++;                                // wait for port activation
    GPIO_PORTB_DIR   |= 0xFF;              // make PB0-7 outputs
    GPIO_PORTB_AFSEL &= ~(0xFF);           // disable alternate functions 
    GPIO_PORTB_DEN   |= 0xFF;              // enable digital I/O on PB0-7
    
                                          // activate control pins
    RCGCGPIO |= 0x01;                     // activate port A
    RCGC2 |= 0x01;
    wait++;                               // wait for port activation
    wait++;                               // wait for port activation
    GPIO_PORTA_DIR   |= 0xF0;             // make PA4-7 outputs
    GPIO_PORTA_AFSEL &= ~(0xF0);          // disable alternate functions 
    GPIO_PORTA_DEN   |= 0xF0;             // enable digital I/O on PA4-7
    GPIO_PORTA_DATA  |= 0xF0;             //  intialize all control signal
